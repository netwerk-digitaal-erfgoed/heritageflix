<template>
  <div class="page absolute z-10 bg-white h-screen w-screen border-blue grid" :class="transition">
    ...
    <div class="row-start-2 h-full w-full overflow-hidden grid grid-cols-12 grid-rows-12">
      <MoleculesIconNavigation name="arrowLeft" :to="navPath(true)" v-if="state.prev" class="col-span-1 flex justify-center items-center row-start-1 row-span-12 col-start-1" />
      ...
      <div class="col-start-2 col-span-6 row-start-9 row-span-4 flex flex-col m-4 mr-0" ref="descriptionContainer">
        <h2 class="text-4xl" ref="titleElement">{{ currentArtwork?.title }}</h2>
      ...
      <MoleculesIconNavigation name="arrowRight" :to="navPath()" v-if="state.next" class="col-span-1 flex justify-center items-center row-start-1 row-span-12 col-start-12" />
    </div>
  </div>
</template>
